(() => {
  ...
  if (bookModal) {
    ...
    function buildIcsDataUri(start, end) {
      ...
      const uid = 'ks-' + Math.random().toString(36).slice(2) + '@kapsalonsamir.nl';
      const summary = `Kapsalon Samir — ${state.serviceLabel}`;
      const description = `Afspraak: ${state.serviceLabel}\\nStylist: ${state.barberLabel}\\nNaam: ${state.name}\\nPrijs: €${state.price}\\n\\nGroenestraat 277, 6531 HK Nijmegen`;

      const ics = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Kapsalon Samir//Booking//NL',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        'BEGIN:VEVENT',
        `UID:${uid}`,
        ...
        'LOCATION:Groenestraat 277\\, 6531 HK Nijmegen',
        'END:VEVENT',
        'END:VCALENDAR'
      ].join('\r\n');
